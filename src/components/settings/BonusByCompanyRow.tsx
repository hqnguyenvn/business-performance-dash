...
interface BonusByCompanyRowProps {
  row: BonusByCompany;
  idx: number;
  companies: MasterData[];
  editingCell: { id: string; field: keyof BonusByCompany } | null;
  onEditCell: (id: string, field: keyof BonusByCompany) => void;
  onBlurCell: () => void;
  saveCell: (id: string, field: keyof BonusByCompany, value: any) => void;
  onDelete: (id: string) => void;
  onAddRowAfter: (id: string) => void;
}

export const BonusByCompanyRow: React.FC<BonusByCompanyRowProps> = ({
  row, idx, companies, editingCell, onEditCell, onBlurCell, saveCell, onDelete, onAddRowAfter
}) => {
  const isEditing = (field: keyof BonusByCompany) => editingCell?.id === row.id && editingCell.field === field;
  ...
  const handleCellSave = (field: keyof BonusByCompany, value: any) => {
    if (value !== row[field]) {
  ...
  return (
    <TableRow className="group h-[53px]">
      ...
      <TableCell className="text-center p-1 border border-gray-300">
        {isEditing("company_id") ? (
          <Select
            value={tempValue ?? ""}
            onValueChange={value => {
              setTempValue(value);
              handleCellSave("company_id", value);
            }}
            open={true}
          >
          ...
          <div className="cursor-pointer h-8 flex items-center justify-center" onClick={() => handleCellClick("company_id")}>
            {companies.find(c => c.id === row.company_id)?.code ?? ""}
          </div>
        )}
      </TableCell>
...
export default BonusByCompanyRow;
